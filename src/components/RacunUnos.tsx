...
export function RacunUnos({ onUspjeh }: RacunUnosProps) {
  ...
  const [kreiraniRacun, setKreiraniRacun] = useState<{
    idRacuna: number; brojRacuna: string; referentniBroj: string;
    datumRacuna: string; partner: Partner; stavkeSnimak: StavkaRacuna[]; napomena: string; ukupno: number;
  } | null>(null);
  ...
  return (
    ...
    >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-stretch">
                      ...
                      <div className="flex-1 min-w-0 px-4 pt-3 pb-3 flex flex-col justify-between">
                        <div className="min-w-0">
                          <span className="text-[10px] font-bold font-mono block" style={{ color: PRIMARY }}>
                        ...
                        <div className="rounded-lg px-2.5 py-1 mt-2 self-start bg-white dark:bg-[#0f2320] border border-gray-100 dark:border-[#1a3d38] text-[11px]">
                          ...
                          <span className="font-bold" style={{ color: ACCENT }}>{s.mpc.toFixed(2)}</span>
                        </div>
                      </div>
                      ...
                      >
                        <span className="text-2xl font-extrabold leading-none" style={{ color: PRIMARY }}>
                          {s.kolicina}
                        </span>
                        {s.artikal.jm && (
                          <span className="text-[11px] font-semibold mt-0.5 uppercase tracking-wide" style={{ color: `${PRIMARY}99` }}>
                            {s.artikal.jm}
                          </span>
                        )}
                      </div>
          ...
          >
            <div className="flex-1 flex flex-col">
              ...
              <span className="text-base font-bold" style={{ color: stavke.length > 0 ? PRIMARY : "#d1d5db" }}>
      ...
      {pokaziPotvrdu && ReactDOM.createPortal(
        <div
          className="fixed inset-0 z-[9999] flex items-center justify-center"
          style={{ background: "rgba(0,0,0,0.5)" }}
          onClick={() => { if (!kreiraniRacun) { setPokaziPotvrdu(false); setNapomena(""); } }}
        >
          <div
          ...
          {kreiraniRacun ? (
            ...
            <div className="px-6 py-8 flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-gray-800 dark:text-[#e6f4f2]">Račun uspješno kreiran</h3>
                <p className="text-2xl font-extrabold mt-1" style={{ color: PRIMARY }}>{kreiraniRacun.brojRacuna}</p>
                <p className="text-sm text-gray-500 dark:text-[#4a7a74] mt-1">{kreiraniRacun.partner.naziv_partnera}</p>
                <p className="text-sm font-bold mt-0.5" style={{ color: ACCENT }}>
                  {kreiraniRacun.ukupno.toLocaleString("bs-BA", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} KM
                </p>
              </div>
              <div className="flex gap-3 w-full pt-2">
                <button
                  onClick={() => { setPokaziPotvrdu(false); setKreiraniRacun(null); setNapomena(""); }}
                  className="flex-1 py-2.5 text-sm font-semibold rounded-xl border border-gray-200 dark:border-[#1e4a44] text-gray-600 dark:text-[#4a7a74] hover:bg-gray-50 dark:hover:bg-[#1a3d38] transition-all"
                >
                  Zatvori
                </button>
            ...
            <div className="px-6 pt-6 pb-4 flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  <Building2 size={16} style={{ color: PRIMARY }} className="flex-shrink-0" />
                  <div className="min-w-0">
                ...
                <div className="mt-3 flex items-center gap-4 text-sm">
                  <span className="text-gray-400 dark:text-[#4a7a74]">
                    {stavke.length} {stavke.length === 1 ? "stavka" : "stavki"}
                  </span>
              ...
              <button
                onClick={() => { setPokaziPotvrdu(false); setNapomena(""); }}
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              >
                {slanjePodataka
                  ? <><Loader2 size={15} className="animate-spin" /> Kreiranje...</>
