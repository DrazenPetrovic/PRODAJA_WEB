    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-gray-800 dark:text-[#e6f4f2]">Pregled računa</h2>
            <p className="text-[10px] text-gray-400 dark:text-[#4a7a74]">{zaglavlja.length} računa ukupno</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {zaglavlja.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            <FileText size={36} className="mb-3 opacity-10 dark:opacity-5" style={{ color: PRIMARY }} />
            <p className="text-sm text-gray-300 dark:text-[#2a5a54]">Nema unesenih računa</p>
          </div>
        ) : (
            ...
            {zaglavlja.map((r) => {
              ...
              return (
                <li
                  ...
                  style={{ borderColor: storno ? `${ACCENT}55` : `${PRIMARY}40` }}
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ background: storno ? `${ACCENT}18` : `${PRIMARY}12` }}
                    >
                      <FileText size={15} style={{ color: storno ? ACCENT : PRIMARY }} />
                    </div>
                  ...
                  {otvoren && (
                    <div className="border-t-2 dark:border-[#1a3d38] bg-gray-50 dark:bg-[#0a1e1c]"
                      style={{ borderColor: storno ? `${ACCENT}55` : `${PRIMARY}40` }}
                    >
                      <div className="px-5 py-4 grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          {stavke.length === 0 ? (
                            <p className="text-xs text-gray-400 dark:text-[#4a7a74]">Nema stavki</p>
                          ) : (
                            <table className="w-full text-xs">
                              ...
                              <tfoot>
                                <tr className="border-t-2 border-gray-200 dark:border-[#1e4a44]">
                                  <td colSpan={3} className="pt-2 text-right font-bold text-gray-600 dark:text-[#a8d5cf]">
                                    Ukupno:
                                  </td>
                        ...
                        <div className="space-y-2 pl-4 border-l border-gray-200 dark:border-[#1a3d38]">
                          ...
                          {[
                            ...
                            ...(r.id_originalnog_racuna
                              ? [["Originalni račun ID", String(r.id_originalnog_racuna)]]
                              : []),
                          ].map(([label, value]) => (
                            <div key={label} className="flex items-start gap-2">
                              <span className="text-[11px] text-gray-400 dark:text-[#4a7a74] w-32 flex-shrink-0">{label}:</span>
                              <span className="text-[11px] font-semibold text-gray-700 dark:text-[#c5e0db]">{value}</span>
                            </div>
                          ))}
                          {r.napomena_operatera && (
                            <div className="mt-2 pt-2 border-t border-gray-200 dark:border-[#1a3d38]">
                              <p className="text-[10px] text-gray-400 dark:text-[#4a7a74] mb-0.5">Napomena:</p>
                            ...
                            onClick={() => setPrintJob({
                              ...
                              component: (
                                <RacunTemplate
                                  racun={r}
                                  stavke={stavke}
                                  nazivPartnera={partneriMap.get(r.id_partnera) ?? `ID ${r.id_partnera}`}
                                />
                              ),
                            })}
